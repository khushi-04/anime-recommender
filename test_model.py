...
def run_pipeline(data_fraction=1.0):
    start = time.time()

    spark = SparkSession.builder.appName("AnimeALSRecommendation") \
    ...
    ratings_clean = ratings.sample(fraction=data_fraction, seed=42) \
    ...
    exploded = top_recommendations.withColumn("rec", explode("recommendations")) \
...
for frac in fractions:
    duration = run_pipeline(data_fraction=frac)
    print(f"Data: {int(frac*100)}%, Time: {duration}s")
